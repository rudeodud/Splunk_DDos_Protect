...
VPC Flow Logs             -> Firehose             -> S3 -> Splunk
```

CloudFront 실시간 로그는 Kinesis Data Stream으로 전달되고, Firehose가 이를 S3에 적재합니다. AWS WAF 로그는 WAF logging configuration을 통해 Firehose로 직접 전달됩니다. VPC Flow Logs는 VPC 내부 네트워크 흐름의 `srcaddr`, `dstaddr`, `dstport`, `packets`, `bytes`, `action` 정보를 Firehose를 통해 S3에 적재합니다. 로컬 Splunk Enterprise는 S3에 저장된 로그를 주기적으로 수집하여 인덱싱합니다.
...
- **S3**: CloudFront/WAF/VPC Flow Logs의 중앙 저장소입니다.
- **Local Splunk Enterprise**: 발표자 PC에서 실행되며 Splunk Add-on for AWS로 S3 로그를 수집합니다. Terraform은 Splunk EC2를 생성하지 않습니다.
